...
class ORCNNROIHeads(ROIHeads):
    ...
    def forward(
        ...
        if self.training:
            ...
            losses, box_head_features, _inner_bin = self._forward_box(features_list, proposals);    inner_bin.update(_inner_bin)
            amodal_vis_occ_losses, _inner_bin = self._forward_masks(features, proposals, box_head_features);    inner_bin.update(_inner_bin)
            losses.update(amodal_vis_occ_losses)
            
            # return proposals, losses
            return proposals, losses, inner_bin
    ...
    def _forward_box(self, features: List[torch.Tensor], proposals: List[Instances]
        ...
        if self.occ_cls_at_box:     # False by default
        ...
        inner_bin['roi_feature'] = box_features
        inner_bin['box_feature'] = box_head_features
        del box_features
        ...
        if self.training:
            ...
            return outputs.losses(), box_head_features, inner_bin
        else:
        ...
        if pred_target == "V":
            ...
            for p in proposals:
                if p.has("gt_occluded_rate"):
                    _gt_occludeds.append(p.gt_occluded_rate >= 0.05)
            if _gt_occludeds == []:
                losses["loss_occ_cls"] = torch.tensor(0)
            else:
                gt_occludeds = cat(_gt_occludeds, dim=0).to(torch.int64)
...
#
        if self.training:
            ...
            inner_bin['roi_feature_fg'] = features
            ...
            inner_bin['post_feature'] = mask_features_list
            inner_bin['output_logits'] = logits
            inner_bin['proposals'] = proposals
