...
def mask_rcnn_inference(pred_mask_logits, pred_instances, target="pred_masks"):
    ...
    if cls_agnostic_mask:   # If is class logits...
        mask_probs_pred = pred_mask_logits.sigmoid()
    else:
        # Select masks corresponding to the predicted classes
        num_masks = pred_mask_logits.shape[0]
        class_pred = cat([instance.pred_classes for instance in pred_instances])
        indices = torch.arange(num_masks, device=class_pred.device)
        mask_probs_pred = pred_mask_logits[indices, class_pred][:, None].sigmoid()
    # mask_probs_pred.shape: (B, 1, Hmask, Wmask)

    num_boxes_per_image = [len(instance) for instance in pred_instances]
