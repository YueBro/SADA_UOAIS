...
from ..rcnn.mask_heads import *
from ..rcnn.faster_rcnn import FastRCNNOutputs
from ..rcnn.faster_rcnn import FastRCNNOutputLayers
from .bbox_pooler_explainer import BBOXROIPooler
from .pooler_explainer import ROIPooler
from ..rcnn.box_head import build_box_head
from detectron2.utils.events import get_event_storage

from .explainer_tools import styler
...
class DIY_ORCNNROIHeads(ROIHeads):
    """
    ...
    def __init__(self, cfg, input_shape):
        super(DIY_ORCNNROIHeads, self).__init__(cfg, input_shape)
    ...
    def forward(
        ...
        self._str = "Parameters:\n"
        self._str += f"\tpooler_resolution = {self.cfg.MODEL.ROI_BOX_HEAD.POOLER_RESOLUTION}\n"
        self._str += f"\tpooler_scales = {tuple(1.0 / self.feature_strides[k] for k in self.in_features)}\n"
        self._str += f"\tsampling_ratio = {self.cfg.MODEL.ROI_BOX_HEAD.POOLER_SAMPLING_RATIO}\n"
        self._str += f"\tpooler_type = {self.cfg.MODEL.ROI_BOX_HEAD.POOLER_TYPE}\n"
        self._str += f"\tnum_classes = {self.num_classes} ({self.num_classes+1} actually)\n\n"

        self._str += "Inputs:\n"
        self._str += f"\tfeatures\n\t\tkeys = {tuple(features.keys())}\n"
        self._str += f"\tproposals\n\t\t[Instances(image_size: {type(proposals[0].image_size).__name__}, proposal_boxes: {type(proposals[0].proposal_boxes).__name__})]\n\n"
        ...
        if self.training:
            ...
            (losses, box_head_features), _str = self._forward_box(features_list, proposals)
            amodal_vis_occ_losses, _str = self._forward_masks(features, proposals, box_head_features)
            losses.update(amodal_vis_occ_losses)
            
            return (proposals, losses), self._str

        else:
            # During inference cascaded prediction is used: the mask and keypoints heads are only
            # applied to the top scoring box detections.
            features_list = [features[f] for f in self.in_features]
            (pred_instances, box_head_features), _str = self._forward_box(features_list, proposals)
            self._str += "=" * 50 + "\n"
            self._str += "self._forward_box:\n"
            self._str += "-+[START]" + "-" * 41 + "\n"
            self._str += " | " + _str.replace("\n", "\n | ")[:-3]
            self._str += "-+[END]" + "-" * 43 + "\n\n"

            pred_instances, _str = self._forward_masks(features, pred_instances, box_head_features)
            self._str += "=" * 50 + "\n"
            self._str += "self._forward_masks:\n"
            self._str += "-+[START]" + "-" * 41 + "\n"
            self._str += " | " + _str.replace("\n", "\n | ")[:-3]
            self._str += "-+[END]" + "-" * 43 + "\n\n"

            self._str += "=" * 50 + "\n"
            self._str += "Final Output:\n"
            self._str += "\tpred_instances:\n"
            __keys = pred_instances[0].get_fields()
            __l = max([len(k) for k in __keys])
            __s = "{" + f":<{__l}" + "}"
            for key in __keys:
                element = pred_instances[0].get(key)
                if isinstance(element, Boxes):
                    element = element.tensor
                self._str += f"\t\t.{__s.format(key)} shape={tuple(element.shape)}\n"
            return (pred_instances, {}), self._str
    ...
    def _forward_box(self, features: List[torch.Tensor], proposals: List[Instances]
        ...
        box_features, __str = self.box_pooler(features, [x.proposal_boxes for x in proposals])
        _str += "Extract features by proposals (self.box_pooler):\n"
        _str += f"\t- Process: [features, proposals] -> (ROIAlign) -> box_features({tuple(box_features.shape)})\n"
        _str += f"\t- Model class name: \"{type(self.box_pooler).__name__}\"\n\n"
        _str += "\t"+__str.replace("\n", "\n\t")[:-1]
        _str += "\t(box_features <- output)\n\n"

        box_head_features, first_head_features = self.box_head(box_features)    # /adet/modeling/rcnn/box_head.py -> MLCFastRCNNConvFCHead
        _str += "BoxHead convolutions: (self.box_head)\n"
        _str += "\t- Process: box_features -> (CONV) -> first_head_features -> (CONVs) -> box_head_features\n"
        _str += "\t- Comment: add some layers.\n"
        _str += f"\t- Model class name: \"{type(self.box_head).__name__}\"\n"
        _str += "\t- Model info:\n\t\tconv_dims=[256, 256, 256, 256]\n\t\tfc_dims=[]\n\n"
        _str += f"\t\tbox_head_features: {type(box_head_features).__name__}{tuple(box_head_features.shape)}\n"
        _str += f"\t\tfirst_head_features: {type(first_head_features).__name__}{tuple(first_head_features.shape)}\n"
        _str += "\n"

        pred_class_logits, pred_proposal_deltas = self.box_predictor(box_head_features)
        _str += "Bounding Box Branch (self.box_predictor)\n"
        _str += f"\n\t\t\033[1m(C)\033[0m pred_class_logits:  {type(pred_class_logits).__name__}(size={tuple(pred_class_logits.shape)})\n"
        _str += f"\t\t\033[1m(B)\033[0m pred_proposal_deltas:  {type(pred_proposal_deltas).__name__}(size={tuple(pred_proposal_deltas.shape)})\n"
        ...
        outputs = FastRCNNOutputs(
            ...
            pred_occlusion_class_logits = pred_occ_cls_logits,
        )
        ...
        if self.MLC:
            ...
            _str += "\n"
            _str += "Upsampling Convs:\n"
            _str += f"\t- Process: box_head_features -> (ConvTranspose2d) -> (CONV)×{len(self.mlc_layers)-1} -> box_head_features\n"
            _str += f"\n\t\tbox_head_features: {type(box_head_features).__name__}(size={tuple(box_head_features.shape)})\n"
        else:
        ...
        if self.training:
            ...
            return (outputs.losses(), box_head_features), _str
        else:
            ...
            _str += f"Output:\n"
            _str += "\t- Comment: Select out the boxes with foreground_logits ≥ \"self.test_nms_thresh\".\n"
            _str += f"\t- Relavent Parameters:\n"
            _str += f"\t\tself.test_score_thresh = {self.test_score_thresh}\n"
            _str += f"\t\tself.test_nms_thresh = {self.test_nms_thresh}\n"
            _str += f"\t\tself.test_detections_per_img = {self.test_detections_per_img}\n"

            _str += f"\t- Resulted variables:\n"
            _str += f"\t\t\033[91mpred_instances\033[0m[0]: type={pred_instances[0].__class__.__name__}\n"
            _str += f"\t\t\t.pred_boxes:   type={pred_instances[0].pred_boxes.__class__.__name__}, shape={tuple(pred_instances[0].pred_boxes.tensor.shape)}\n"
            _str += f"\t\t\t.scores:       {pred_instances[0].scores}\n"
            _str += f"\t\t\t.pred_classes: {pred_instances[0].pred_classes}\n"
            _str += f"\t\tindex (indicates which box): {index[0]}\n"
            _str += f"\t\t\033[91mbox_head_features\033[0m = box_head_features[index] \033[95m(slicing)\033[0m\n"

            _str += "\n"
            _str += "Final Returned Variables:\n"
            _str += f"\t{styler.red}pred_instances{styler.reset}\n"
            _str += f"\t{styler.red}box_head_features{styler.reset}\n"
            return (pred_instances, box_head_features), _str
    ...
    def _inference_single_mask(self, pred_target, features, mask_features_list, 
        ...
        _str += f"{styler.under}input_features.shape = {tuple(input_features.shape)}{styler.reset}\n"
        __hist_keys = list(instances[0].get_fields().keys())

        if pred_target == "V":
            mask_logits, output_features = self.visible_mask_head(input_features, instances, box_head_features)
            _str += f"mask_logits.shape = {tuple(mask_logits.shape)}\n"
            _str += f"output_features.shape = {tuple(output_features.shape)}\n"
            mask_rcnn_inference(mask_logits, instances, "pred_visible_masks")
            _str += list(instances[0].get_fields().keys()).__str__() + "\n"
            logits["visible"] = mask_logits
            ...
            _str += f"occ_cls_logits.shape = {tuple(occ_cls_logits.shape)}\n"
            _str += f"output_features.shape = {tuple(output_features.shape)}\n"
            _str += f"occ_probs.shape = {tuple(occ_probs.shape)}\n"
        
        _str += f"{styler.blue}instances[0] attributes:{styler.reset}\n"
        __l = max([len(k) for k in instances[0].get_fields()]) + 1
        __s = "{}:{}{}".format("{",__l,"}")
        for key in instances[0].get_fields():
            element = instances[0].get(key)
            if isinstance(element, Boxes):
                element = element.tensor
            
            if key in __hist_keys:
                _str += f"\t{styler.blue}" + __s.format(f"{key}") + f"shape={tuple(element.shape)}{styler.reset}\n"
            else:
                _str += f"\t{styler.blue_bold}" + __s.format(f"{key}") + f"shape={tuple(element.shape)}{styler.reset}\n"

        return instances, output_features, logits, _str
    ...
    def _forward_masks(self, features, instances, box_head_features):
        ...
        if self.training:
            ...
            for pred_target in self.prediction_order:
                losses, mask_features, logits, __str = \
            ...
            features, __str = self.mask_pooler(features, instances)
            _str += "Extract features by proposals (self.mask_pooler):\n"
            _str += "\t" + __str.replace("\n", "\n\t")[:-1]
            _str += "\n"


            _str += "Masks Prediction:\n"
            _str += "\t- Comment: This step predicts the masks in a hirechical order.\n"
            _str += "\t           The order in paper is (V -> A -> O).\n"
            _str += "\t- Important Parameters:\n"
            _str += f"\t\tself.prediction_order = {self.prediction_order}\n\n"
            ...
            for pred_target in self.prediction_order:
                _, mask_features, logits, __str = \
                    self._inference_single_mask(pred_target, features, mask_features_list,\
                                            instances, box_head_features, logits)
                mask_features_list.append(mask_features)

                _str += f"\t\tMask {styler.bold_pink}{pred_target}{styler.reset}:\n"
                _str += "\t\t\t"+__str.replace("\n", "\n\t\t\t")[:-3] if __str!="" else ""
