...
class ROIPooler(nn.Module):
    ...
    def forward(self, x, instances, is_train=False):
        ...
        """
        _str = ""
        _str += "- Comment: Same as \"self.box_pooler\" in \"self._forward_box()\".\n"
        _str += f"- Model class name: \"{type(self).__name__}\"\n"

        if is_train:
            box_lists = [instance.proposal_boxes for instance in instances]
        else:
            box_lists = [instance.pred_boxes for instance in instances]
        ...
        _str += "- Final output shape:\n"
        _str += f"\t{tuple(output.shape)}\n"
