...
# My libraries
from eval.eval_utils.munkres import Munkres
...
def multilabel_metrics(prediction, gt, obj_detect_threshold=0.75, return_assign=False):
    ...
    for i, gt_i in enumerate(labels_gt):
        ...
        for j, pred_j in enumerate(labels_pred):
            ...
            # true positive
            tp_mask = np.logical_and(pred_j_mask, gt_i_mask)
            tp = np.int64(np.count_nonzero(tp_mask)) # Cast this to numpy.int64 so 0/0 = nan
